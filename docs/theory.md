...
*   **理想推定量 $\hat{\boldsymbol{\zeta}}_{N}^{\mathrm{C}}$ （連続観測に基づく）**
    物理現象としての連続値 $\boldsymbol{y}_{n}^{\mathrm{C}}$ に直接アクセスできると仮定した場合の推定量である。
    *   **定義**: $\hat{\boldsymbol{\zeta}}_{N}^{\mathrm{C}} = T(F_{N, y})$
    *   **MLEの具体例**: $\hat{\boldsymbol{\xi}}_{N, \mathrm{MLE}}^C = \arg \max_{\boldsymbol{\xi}} \sum_{n=1}^N \log g(\boldsymbol{y}_n - h(\boldsymbol{x}_n, \boldsymbol{\beta}) \mid \boldsymbol{\theta})$
...
## 非有界連続測定データにおけるロバスト推定手法の体系的枠組み：M、L、R推定
...
## 応答変数が量子化されているデータにおけるロバスト推定

従来のロバスト統計学の多くは、応答変数の測定データが任意の精度を持つ実数（連続極限）である、あるいはそのような極限に近似できることを前提としていた。しかし、デジタルシステムの物理的制約、通信帯域の制限、あるいは計算コストの削減といった要請から、データは「量子化（Quantization）」という不可逆な離散化プロセスを経て処理される。量子化データに対するロバスト推定の研究は、単なる精度低下への対応ではなく、離散化がもたらす統計的なバイアスと、データ汚染への脆弱性をいかに同時に克服するか、という極めて現代的な課題を扱っている[^6][^8]。
...
## 説明変数が量子化されているデータにおけるロバスト推定

### 説明変数が量子化されることによって引き起こされる問題

#### 既存の手法が破綻する具体例
非線形な物理モデル $y = h(x, \beta)$ を用いてセンサーデータの解析を行う際、説明変数 $x$ が粗く量子化されていると、従来のロバスト推定は正常なデータを外れ値と誤認する。例えば、真のモデル曲線付近にあるデータであっても、説明変数の量子化によって計算上の残差が局所的に増大する場合がある。このとき、Huber損失などのロバストな損失関数を用いると、量子化誤差による大きな残差を持つ正常なデータ点に対して小さな重みが割り当てられ、有効な情報が棄却される。

#### 破綻が起きる仕組み
破綻の主な原因は、量子化誤差がモデルの非線形性を介して構造的なバイアスを生じさせる点にある。
量子化された説明変数 $x^\mathrm{Q}$ と連続値である真値 $x^\mathrm{C}$ の間には、量子化誤差 $\delta = x^\mathrm{Q} - x^\mathrm{C}$ が存在する。回帰モデル $h(x, \beta)$ を $x^\mathrm{C}$ の周りでテイラー展開すると、見かけの残差 $\tilde{r}$ は次のように近似される。
$$\tilde{r} = y - h(x^\mathrm{Q}, \beta) \approx \varepsilon - \nabla_x h(x^\mathrm{C}, \beta)^\top \delta$$
ここで $\varepsilon$ は観測ノイズである。第2項の $\nabla_x h(x^\mathrm{C}, \beta)^\top \delta$ は量子化に由来する誤差であり、これは単なるランダムノイズではなく説明変数の値に依存する。
ロバスト推定において重みを決定する影響関数は残差の大きさに依存するため、この第2項が大きくなる領域では、観測ノイズ $\varepsilon$ が小さい正常なデータであっても、重みが不当に下げられる。また、期待値 $\mathbb{E}[\tilde{r} \mid x^\mathrm{Q}]$ が $0$ にならないため、推定値 $\hat{\beta}$ に系統的なバイアスが生じる。

### 問題への対応策
量子化誤差の影響を陽に考慮した積分型損失関数の導入が有効である。
従来の損失関数 $\rho(y - h(x^\mathrm{Q}, \beta))$ の代わりに、量子化区間 $\mathcal{I}_{x^\mathrm{Q}}$ における期待損失を用いる。
$$L(\beta) = \sum_{n=1}^N \int_{\mathcal{I}_{x_n^\mathrm{Q}}} \rho(y_n - h(x, \beta)) p(x \mid x_n^\mathrm{Q}) dx$$
この定式化により、量子化区間内でのモデルの変動を平均化して評価することが可能となり、特定の点での量子化誤差による重みの誤設定を抑制できる。解析的な積分が困難な場合は、モンテカルロ積分や数値積分による近似を用いることで、数学的な妥当性と実用性を両立させる。

---

## 応答変数が多次元のデータにおけるロバスト推定

### 応答変数が多次元になることによって引き起こされる問題

#### 既存の手法が破綻する具体例
多変量応答変数 $\boldsymbol{y} \in \mathbb{R}^L$ を扱う際、各次元に対して独立に1次元のロバスト推定を適用すると、次元間の相関を破壊する外れ値を検知できない。例えば、各次元の成分自体は正常な範囲内（例：$\pm 2\sigma$ 内）にあっても、変数間の本来の相関関係から大きく逸脱しているデータ点が存在する場合、単変量の手法ではこれを外れ値として処理できず、共分散構造の推定に大きな誤差が生じる。

#### 破綻が起きる仕組み
多次元空間における外れ値は、周辺分布ではなくマハラノビス距離 $d^2 = (\boldsymbol{y} - \boldsymbol{\mu})^\top \boldsymbol{\Sigma}^{-1} (\boldsymbol{y} - \boldsymbol{\mu})$ によって定義されるべき性質を持つ。
次元数 $L$ が増大すると、空間の疎性が増し、データが「中心」から離れる確率が指数関数的に高まる。このとき、特定の次元だけに依存するロバスト手法では、$\boldsymbol{\Sigma}^{-1}$ に含まれる相関情報（非対角成分）を考慮できないため、構造的な外れ値を適切に重み付けできない。
また、応答変数が独立に量子化されている場合、多次元空間上ではデータが格子状に配置される。これにより、ベクトルとしての方向情報が歪められ、真の共分散構造が持つ滑らかな依存関係が量子化バイアスによって遮蔽される。

### ### 問題への対応策
多変量M-推定に基づき、回帰係数 $\boldsymbol{\beta}$ とロバストな共分散行列 $\boldsymbol{\Sigma}$ を同時に最適化するアプローチをとる。
損失関数をマハラノビス距離の関数 $\rho(d_n)$ として定式化し、外れ値の寄与をベクトル単位で制御する。計算効率とロバスト性を両立させるため、初期値として最小共分散決定法（MCD）などの崩壊点の高い手法を用い、反復再重み付け最小二乗法（IRLS）によって解を求める。
また、量子化による相関の損失に対しては、コピュラ（Copula）を用いて周辺分布の量子化影響と依存構造を分離してモデル化することで、多次元的な相関情報を保護しつつロバストな推定を実現する。
